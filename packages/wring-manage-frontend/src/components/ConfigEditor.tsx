  ...
  return (
    <Form onSubmit={() => {
      ...
      <Form.Field>
        ...
        <Form.Input type="text" value={name} onChange={e => setName(e.currentTarget.value)} />
      </Form.Field>
      <Form.Field>
...
function ConfigFormContainer({ config }: ConfigFormContainerProps) {
  ...
  const [configValid, setConfigValid] = useState(() => !!config && isValidWebRingConfig(config.data));
  ...
  return (
    <>
      ...
      <ConfigForm key={key} initialValues={
        (config && configValid) ? {
          name: config.data.name,
          prev: config.data.prev.length > 0 ? config.data.prev[0].site : "",
          next: config.data.next.length > 0 ? config.data.next[0].site : ""
        } : emptyConfigValues
      }
      onSubmit={values => {
        writeConfig({
...
function GetStarted({ onClick }: { onClick: () => void; }) {
  return (
    <>
      <p>
        It looks like you don't have a web ring config yet.
      </p>
